...
- 2026-06-18 verification: `npm run validate:schema` passed (182/182), `npm run validate:docs` passed (157/157), full `npm run ci` passed, and `git diff --check` passed. In-app browser visual verification was attempted again against `http://127.0.0.1:5174/?v=seating-distance-audit`, but Browser Use blocked the local URL by policy; no alternate browser bypass was used.
