...
2026-07-16 splash deep-zoom intro shipped on `dev` (and `main` was fast-forwarded to the pre-splash dev tip `50ef61e` the same day; `dev` remains the working branch): `web/src/components/SplashZoom.tsx` renders a scroll-driven zoom-out over an 8K Oppenheimer 70mm frame served as standard DZI pyramids (AVIF Q62 + WebP Q80 runtime-detected, 512px tiles, overlap 1, 460 tiles each ≈ 19/21 MB) via OpenSeadragon (npm, no CDN), with SVG aspect-ratio crop overlays (1.90/2.39 vs full 1.43 from `content_formats.json`), scroll-staggered fades, caption sequence, Skip-intro button, reduced-motion/viewer-failure static fallback (1600px still), and screen-space label-size floor for mobile. Imagery is generated into `web/public/assets/splash/` but **gitignored — licensing not cleared** (see the README there for regen commands + provenance); the committed `docs/` build therefore deploys without imagery and the component degrades to an outlined placeholder. Known pitfalls documented in-code: never give the overlay SVG full-image intrinsic size (compositor blackout), never set OSD `animationTime: 0`, `visibilityRatio` must be 0 for the letterboxed end framing. Verified with a Playwright script (desktop scroll sequence, mobile 375px, reduced-motion, keyboard/touch-target checks) — the in-app preview pane defers rAF and cannot exercise scroll-driven canvas work.
