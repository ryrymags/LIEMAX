...
Current status: "Steps 1–3 audited and clean; data layer complete. **Overhaul V2 Milestone 1 shipped 2026-07-15**: the old `docs/` prototype is retired (preserved at git tag `prototype-final`; harvest prototype at `research/archive/prototypes/`) and `docs/` is now the committed Vite build output of the new `web/` app (Vite 8 + React 19 + TS; imports `src/math` directly; data via generated `src/data/generated/docs_bundle.json`). M1 = nav, splash with honest stat strip, ARIA-combobox theater search, Layer 1 zero-jargon verdict card (four tiers + TRUE-IMAX sub-states, beginner copy, human-vs-screen scale diagram, what-you're-missing region callout, two-path CTA) in neutral token-only styling. Next: the Claude Design loop — user runs a separate Claude Design chat with `design/DESIGN_BRIEF_M1.md`; implementing its output is M1.5, then M2 = tooltips + IMAX 101 blocks (content authoring per Bible §4.3). The 3D POV module ports in M4 using the original test pattern at `web/public/assets/pov/liemax-143-test-pattern.svg`. New validation chain: `npm run ci` = typecheck ×2, math (154), schema (182), compare (26), dolby (28), build:web, validate:web (19 bundle/build invariants). validate:docs / validate:math-parity / docs prototype checks retired with the duplicate browser math they guarded."

Current priority: Overhaul V2 — ground-up website redesign per `.ai/OVERHAUL_BIBLE.md`. Data layer is ready; no new data work is needed before starting the build.

2026-07-16/17 splash intro shipped on `dev` (and `main` was fast-forwarded to the pre-splash dev tip `50ef61e` on 07-16; `dev` remains the working branch). The first iteration (OpenSeadragon DZI tile pyramid) was rejected by the owner — visible tile pop-in while scrolling — and replaced: `web/src/components/SplashZoom.tsx` now zooms a SINGLE transformed element (full 1.43:1 frame stacked with a higher-res eye crop, both under one CSS transform, so no handoff) out from film grain, then animates SOLID BLACK letterbox bars stepping 2.39 → 1.90 → 1.43 with per-phase labels, ending on the full frame with labeled reference lines; black pillarboxing comes free from the contain-fit black stage. Assets: `web/public/assets/splash/liemax-{frame-full,eye}-*.{avif,jpg}` (~5 MB total, ~1 MB typical load), regen commands in that folder's README. Imagery is COMMITTED per owner decision 2026-07-16 under a documented fair-use posture (analysis + takedown route in `THIRD_PARTY_NOTICES.md`, exclusion in `LICENSE-CONTENT`, site footer carries non-affiliation + fair-use + removal-request copy from `design/FOOTER_LEGAL_COPY.md`). Visual overhaul pass done same day (warm-dark data-journalism tokens, serif display stack, glowing amber stat strip, gold focus rings, `Footer.tsx`). Pitfalls documented in-code: CSS `aspect-ratio` + `max-height` cannot express contain-fit (JS computes the frame box), bars must reach full black early in the scope hold. Verified with a Playwright script (phase state machine via the DEV-only `window.__liemaxSplash` hook, mobile 375px, reduced-motion, keyboard/touch targets) — the in-app preview pane defers rAF and cannot exercise scroll-driven work.
2026-07-17 splash checkpoint redesign shipped on `dev` (plan: `.ai/SPLASH_CHECKPOINT_PLAN.md`): the splash is now a 900svh scroll timeline with long holds and smootherstep transitions — opening 0–.12, to Scope .12–.24, Scope hold .24–.38, to 1.90 .38–.50, 1.90 hold .50–.64, to 1.43 .64–.76, 1.43 hold .76–.90, comparison .90–1 — each checkpoint carrying HTML copy and a jump CTA, plus a persistent nav "Diagnose my IMAX" anchor to `#diagnose-my-imax`. Opening state frames both eyes of a new TIFF-derived 4400×3300 Oppenheimer crop (`START_CENTER x:4800 y:2700`, `START_WIDTH 4600`, aperture clamp 96–180px, AVIF srcset 1600/2400/3400/4400w + 2400 JPG, `sizes="175vw"`) through a narrow aperture on a pure-black stage (`.splash-zoom-stage` background is `#000`, not the warm token — pillarboxes/bars must stay true black). Canonical bar values 1.43/1.90/2.39 and area-gain copy (1.90 vs Scope 25.8%; 1.43 vs 1.90 32.9%; 1.43 vs Scope 67.1%) are exact and guarded by a static validator, `npm run validate:splash` (41 checks), wired into `npm run ci`. Reduced-motion renders a static comparison whose stage sets `height:auto; overflow:visible` (the sticky-stage rule otherwise clips the caption/CTA invisible) and whose caption is the page H1. Splash H1 → diagnosis H2 hierarchy in both modes. Verified 2026-07-17 via Playwright chrome-channel screenshots at anchors .04/.27/.53/.79/.93, desktop + 375×812, plus CTA landing checks, keyboard focus order, ≥44px targets, zero console/network errors, and reduced-motion desktop/mobile; full `npm run ci` + `git diff --check` green. Known pre-existing nit: footer "Request removal" link is under 44px.
2026-07-10 audit remediation complete (phases 0–7): asymmetric vertical FOV shipped with a 30-check math-parity suite (`npm run validate:math-parity`); stats scoped to current rows with `*_incl_archival` variants; four LFExaminer duplicates suppressed; Providence/dome data corrected (schema 1.6.1); math suite grown to 154 tests with schema-bounds enforcement; seat vertical FOV now uses raked-floor eye height via `eyeHeightAtDistance()` (GT front-row eye ≈ 1/3 up screen); workbench brightness labeled as 2D spec (3D dimming not modeled — tracked); research docs purged of unsupported 16K/18K claims; `netlify.toml` ships security headers (publish dir `docs`) and Three.js has an SRI hash. Remaining tracked item: split "can book 15/70" from the flat-1.43 geometry predicate once an authoritative Odyssey booking-list fixture is added.
