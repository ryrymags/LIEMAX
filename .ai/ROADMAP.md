...
- GREEN `schema/`: v1.6.1 source of truth. Supports all required projector types, projection-mode arrays, dome geometry, sparse 143190 imports, LFExaminer archival source rows, screen width confidence, 3D POV seating geometry fields, home displays, nullable `screen_bottom_height_ft`, and `r_imax_csv`/`lfexaminer` source quality. Multi-wall ScreenX geometry and structured renovation status remain additive future work.
