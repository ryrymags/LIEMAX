...
- GREEN `schema/`: v1.4.0 source of truth. Supports all required projector types, projection-mode arrays, dome geometry, sparse 143190 imports, LFExaminer archival source rows, home displays, and `r_imax_csv`/`lfexaminer` source quality. Multi-wall ScreenX geometry and structured renovation status remain additive future work.
...
4. RESOLVED initial pass: 143190.xyz intentionally excludes venues with only Xenon projectors, so `src/data/fixtures/lfexaminer_us_imax_rows.json` now preserves LFExaminer U.S. IMAX `D`/`1570+D` candidates and `docs/data.js` promotes comparable unmatched rows as `lfexaminer` low-confidence archival 2021 venues. Remaining risk: these rows may be closed/upgraded; future work should add community/official verification and fill eight LFExaminer rows that lack screen dimensions.
